...
class ConfigTests(unittest.TestCase):
    ...
    def test_required_host_strings_are_stripped(self):
        text = (
            VALID_CONFIG_TOML.replace('listen_host = "0.0.0.0"', 'listen_host = " 0.0.0.0 "')
    ...
    def test_minecraft_status_filter_strings_are_stripped(self):
        text = VALID_CONFIG_TOML.replace(
            "recover_after = 2",
            'recover_after = 2\nexpected_version_contains = " 1.21 "\nmotd_must_contain = " READY "\nmotd_must_not_contain = " STARTING "',
        )
        cfg = m.load_config(self.write_temp_config(text))
        self.assertEqual(cfg.healthcheck.expected_version_contains, "1.21")
        self.assertEqual(cfg.healthcheck.motd_must_contain, "READY")
        self.assertEqual(cfg.healthcheck.motd_must_not_contain, "STARTING")

    def test_json_filters_require_valid_json(self):
        text = VALID_CONFIG_TOML.replace('mode = "tcp"', 'mode = "minecraft_status"').replace(
            "recover_after = 2",
    ...
    def test_min_recovery_seconds_validation_and_defaults(self):
        cfg = m.load_config(
            self.write_temp_config(VALID_CONFIG_TOML.replace("min_recovery_seconds = 0.0\n", ""))
        )
        self.assertEqual(cfg.healthcheck.min_recovery_seconds, 0.0)

        for line in [
            "min_recovery_seconds = -1",
            'min_recovery_seconds = "30"',
            "min_recovery_seconds = true",
            "min_recovery_seconds = false",
        ]:
            text = VALID_CONFIG_TOML.replace("recover_after = 2", f"recover_after = 2\n{line}")
            with self.assertRaises(m.ConfigError, msg=line):
                m.load_config(self.write_temp_config(text))
    ...
    def test_validate_config_regressions(self):
        ...
        with self.assertRaises(m.ConfigError):
            m.validate_config(
                m.AppConfig(**{**cfg.__dict__, "proxy": m.ProxyConfig("0.0.0.0", True)})
            )
        with self.assertRaises(m.ConfigError):
            m.validate_config(
                m.AppConfig(
        ...
        with self.assertRaises(m.ConfigError):
            m.validate_config(
                m.AppConfig(**{**cfg.__dict__, "proxy": m.ProxyConfig("0.0.0.0", 70000)})
            )
        with self.assertRaises(m.ConfigError):
    ...
    def test_validate_config_loop_detection_main_fallback_and_healthcheck(self):
        ...
        with self.assertRaises(m.ConfigError):
            m.validate_config(
                m.AppConfig(**{**cfg.__dict__, "main": m.TargetConfig("127.0.0.1", 25565)})
            )
        with self.assertRaises(m.ConfigError):
            m.validate_config(
                m.AppConfig(**{**cfg.__dict__, "fallback": m.TargetConfig("localhost", 25565)})
            )
        bad_hc = m.HealthCheckConfig(
            "tcp",
            3,
            2,
            2,
...
class CoreBehaviorTests(unittest.TestCase):
    def test_proxy_protocol_parse_build_and_trust(self):
        ...
        self.assertTrue(
            m.build_proxy_v1_header("203.0.113.10", "198.51.100.20", 54321, 25565).startswith(
                b"PROXY TCP4 "
            )
        )
        self.assertTrue(
            m.build_proxy_v1_header("2001:db8::1", "2001:db8::2", 54321, 25565).startswith(
                b"PROXY TCP6 "
            )
        )
        with self.assertRaises(ValueError):
    ...
    def test_force_fallback_routes_even_if_main_healthy(self):
        cfg = m.AppConfig(
            **{
                **m.load_config(REPO_ROOT / "config.toml").__dict__,
                "maintenance": m.MaintenanceConfig("force_fallback", None, None),
            }
        )
        state = m.HealthState(2, 2)
        state.set_initial_state(True)
    ...
    def test_force_main_routes_even_if_main_unhealthy(self):
        cfg = m.AppConfig(
            **{
                **m.load_config(REPO_ROOT / "config.toml").__dict__,
                "maintenance": m.MaintenanceConfig("force_main", None, None),
            }
        )
        state = m.HealthState(2, 2)
        state.set_initial_state(False)
    ...
    def test_file_overrides_and_priority_and_dynamic(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            cfg = m.AppConfig(
                **{
                    **m.load_config(REPO_ROOT / "config.toml").__dict__,
                    "maintenance": m.MaintenanceConfig("auto", str(ff), str(fm)),
                }
            )
            self.assertEqual(m.get_effective_maintenance_mode(cfg), ("auto", "auto"))
            fm.touch()
            self.assertEqual(
                m.get_effective_maintenance_mode(cfg), ("force_main", "force_main_file")
            )
            ff.touch()
            self.assertEqual(
                m.get_effective_maintenance_mode(cfg), ("force_fallback", "force_fallback_file")
            )
            ff.unlink()
            self.assertEqual(
                m.get_effective_maintenance_mode(cfg), ("force_main", "force_main_file")
    ...
    def test_choose_target_decision_reason_force_fallback_file(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            cfg = m.AppConfig(
                **{
                    **m.load_config(REPO_ROOT / "config.toml").__dict__,
                    "maintenance": m.MaintenanceConfig("auto", str(ff), None),
                }
            )
            state = m.HealthState(2, 2)
            state.set_initial_state(True)
    ...
    def test_choose_target_decision_reason_force_main_file(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            cfg = m.AppConfig(
                **{
                    **m.load_config(REPO_ROOT / "config.toml").__dict__,
                    "maintenance": m.MaintenanceConfig("auto", None, str(fm)),
                }
            )
            state = m.HealthState(2, 2)
            state.set_initial_state(False)
...
class StatusProtocolTests(unittest.IsolatedAsyncioTestCase):
    async def test_extract_motd_text_variants(self):
        self.assertEqual(m.extract_motd_text("A Minecraft Server"), "A Minecraft Server")
        self.assertEqual(m.extract_motd_text({"text": "Hello"}), "Hello")
        self.assertEqual(
            m.extract_motd_text({"text": "Hello ", "extra": [{"text": "World"}, "!"]}),
            "Hello World!",
        )
        self.assertEqual(m.extract_motd_text([{"text": "READY"}, " Survival"]), "READY Survival")
        self.assertEqual(
            m.extract_motd_text({"extra": [{"text": "READY"}, {"extra": [{"text": " Survival"}]}]}),
            "READY Survival",
        )
        self.assertEqual(
            m.extract_motd_text({"unexpected": 123, "extra": [True, None, {"text": "OK"}]}), "OK"
        )
    ...
    async def test_get_healthcheck_target_overrides(self):
        cfg = m.AppConfig(
            ...
            fallback=m.TargetConfig("127.0.0.1", 25566),
            healthcheck=m.HealthCheckConfig(
                "tcp", 3, 2, 2, 2, 0.0, None, None, 767, None, True, False, 0.0, 0.0, "", "", "", 0
            ),
            connection=m.ConnectionConfig(5.0, 65536, 300.0, False, False, 4096),
            logging=m.LoggingConfig("INFO"),
            maintenance=m.MaintenanceConfig("auto", None, None),
    ...
    async def test_health_loop_recovery_info_is_throttled(self):
        ...
        with (
            mock.patch("mc_failover_proxy.check_main_server", side_effect=fake_check),
            mock.patch(
    ...
    async def test_health_loop_recovery_progress_logs_after_15_seconds(self):
        ...
        with (
            ...
            with self.assertLogs("mc-failover", level="INFO") as captured:
                await m.health_loop(cfg, health, m.RuntimeState(started_at=0.0), stop_event)
        progress_logs = [
            line for line in captured.output if "weiterhin in Recovery-Stabilisierung" in line
        ]
    ...
    async def test_health_loop_logs_recovery_reset_once(self):
        ...
        stop_event = asyncio.Event()
        checks = iter(
            [m.HealthCheckResult(True, "status_json_ok"), m.HealthCheckResult(False, "timeout")]
        )
        ...
        reset_logs = [
            ...
            if "Recovery wurde durch fehlgeschlagenen Healthcheck zurückgesetzt" in line
        ]
    ...
    async def test_health_loop_no_recovery_progress_logs_when_min_recovery_zero(self):
        ...
        checks = iter(
            [
                m.HealthCheckResult(True, "status_json_ok"),
                m.HealthCheckResult(True, "status_json_ok"),
            ]
        )
        monotonic_values = iter([100.0, 103.0])
    ...
    async def test_minecraft_status_healthcheck_paths(self):
        ...
        valid_payload = m.write_varint(
            len(m.write_varint(0) + m.write_varint(len(status)) + status)
        ) + (m.write_varint(0) + m.write_varint(len(status)) + status)
        ...
        bad_id_payload = m.write_varint(len(m.write_varint(1) + m.write_varint(0))) + (
            m.write_varint(1) + m.write_varint(0)
        )
        server, port = await run_server(bad_id_payload)
        try:
            self.assertFalse(
                (await m.minecraft_status_health_check("127.0.0.1", port, 1.0, 767, None, True)).ok
            )
        finally:
            server.close()
            await server.wait_closed()
        ...
        bad_json = b"{notjson"
        bad_json_payload = m.write_varint(
            len(m.write_varint(0) + m.write_varint(len(bad_json)) + bad_json)
        ) + (m.write_varint(0) + m.write_varint(len(bad_json)) + bad_json)
        ...
        try:
            ...
            self.assertTrue(
                (await m.minecraft_status_health_check("127.0.0.1", port, 1.0, 767, None, False)).ok
            )
        finally:
            server.close()
            await server.wait_closed()
        ...
        utf8_payload = m.write_varint(
            len(m.write_varint(0) + m.write_varint(len(invalid_utf8)) + invalid_utf8)
        ) + (m.write_varint(0) + m.write_varint(len(invalid_utf8)) + invalid_utf8)
        ...
        oversized_json_payload = m.write_varint(
            len(m.write_varint(0) + m.write_varint(m.MAX_STATUS_JSON_BYTES + 1))
        ) + (m.write_varint(0) + m.write_varint(m.MAX_STATUS_JSON_BYTES + 1))
        ...
        self.assertFalse(
            (await m.minecraft_status_health_check("127.0.0.1", 9, 0.01, 767, None, True)).ok
        )

    async def test_minecraft_status_healthcheck_filters(self):
        async def run_status(status_obj: dict):
            status = json.dumps(status_obj).encode("utf-8")
            payload = m.write_varint(
                len(m.write_varint(0) + m.write_varint(len(status)) + status)
            ) + (m.write_varint(0) + m.write_varint(len(status)) + status)
        ...
        try:
            self.assertTrue(
                (
                    await m.minecraft_status_health_check(
                        "127.0.0.1", port, 1.0, 767, None, True, expected_version_contains="1.21"
                    )
                ).ok
            )
            self.assertFalse(
                (
                    await m.minecraft_status_health_check(
                        "127.0.0.1", port, 1.0, 767, None, True, expected_version_contains="1.20"
                    )
                ).ok
            )
            self.assertTrue(
                (
                    await m.minecraft_status_health_check(
                        "127.0.0.1", port, 1.0, 767, None, True, motd_must_contain="READY"
                    )
                ).ok
            )
            self.assertFalse(
                (
                    await m.minecraft_status_health_check(
                        "127.0.0.1", port, 1.0, 767, None, True, motd_must_contain="STARTING"
                    )
                ).ok
            )
            self.assertTrue(
                (
                    await m.minecraft_status_health_check(
                        "127.0.0.1", port, 1.0, 767, None, True, motd_must_not_contain="STARTING"
                    )
                ).ok
            )
            self.assertFalse(
                (
                    await m.minecraft_status_health_check(
                        "127.0.0.1", port, 1.0, 767, None, True, motd_must_not_contain="READY"
                    )
                ).ok
            )
            self.assertTrue(
                (
                    await m.minecraft_status_health_check(
                        "127.0.0.1", port, 1.0, 767, None, True, min_players_max=50
                    )
                ).ok
            )
            self.assertFalse(
                (
                    await m.minecraft_status_health_check(
                        "127.0.0.1", port, 1.0, 767, None, True, min_players_max=200
                    )
                ).ok
            )
            self.assertFalse(
                (
                    await m.minecraft_status_health_check(
                        "127.0.0.1", port, 1.0, 767, None, True, max_latency_ms=0.0001
                    )
                ).ok
            )
        finally:
    ...
    async def test_minecraft_status_latency_filter_without_json_required(self):
        async def handler(reader, writer):
            await reader.read(2048)
            payload = m.write_varint(len(m.write_varint(0) + m.write_varint(0))) + (
                m.write_varint(0) + m.write_varint(0)
            )
            writer.write(payload)
            await writer.drain()
            await m.close_writer(writer)
    ...
    async def test_tcp_health_check_timeout_returns_result(self):
        with mock.patch(
            "asyncio.open_connection", new=mock.AsyncMock(side_effect=asyncio.TimeoutError())
        ):
...
class RuntimeIntegrationTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_release_runs_on_connect_failure(self):
        ...
        async def on_connect(reader, writer):
            await m.handle_client(
                cfg, health, limiter, m.RuntimeState(started_at=0.0), reader, writer
            )
    ...
    async def test_main_fail_fallback_success_and_forward(self):
        ...
        cfg = self.make_config(
            9, fb_port, connect_fallback_on_main_connect_failure=True, max_connections=10
        )
        health = m.HealthState(1, 1)
        health.set_initial_state(True)
        limiter = m.ConnectionLimiter(10)
        ...
        fb.close()
        await fb.wait_closed()

    async def test_main_and_fallback_both_unreachable_client_closed(self):
        cfg = self.make_config(
            9, 10, connect_fallback_on_main_connect_failure=True, max_connections=10
        )
        health = m.HealthState(1, 1)
        health.set_initial_state(True)
        limiter = m.ConnectionLimiter(10)
    ...
    async def test_proxy_protocol_send_family_mismatch_falls_back_to_unknown(self):
        ...
        pport = proxy.sockets[0].getsockname()[1]
        with mock.patch(
            "mc_failover_proxy.build_proxy_v1_header", side_effect=ValueError("family mismatch")
        ):
            r, w = await asyncio.open_connection("127.0.0.1", pport)
            w.write(b"PROXY TCP4 203.0.113.10 127.0.0.1 54321 25565\r\nHELLO")
            await w.drain()
...
class MonitoringTests(unittest.IsolatedAsyncioTestCase):
    def _monitoring_config(
        self, force_fallback_file: str | None = None, force_main_file: str | None = None
    ) -> m.AppConfig:
        cfg = m.load_config(REPO_ROOT / "config.example.toml")
        return m.AppConfig(
            **{
    ...
    async def test_monitoring_endpoints(self):
        ...
        state = m.RuntimeState(
            ...
            routing_reason="health_fallback",
        )
        state.last_health_result = m.HealthCheckResult(
            ok=True, reason="status_json_ok", latency_ms=12.3
        )
    ...
    async def test_monitoring_refreshes_immediately_for_maintenance_files(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            try:
                ...
                self.assertIn(
                    'mc_failover_active_target{target="FALLBACK"} 1', await req("/metrics")
                )
    ...
    async def test_monitoring_timeout_returns_408(self):
        ...
        health = m.HealthState(1, 1)
        health.set_initial_state(True)
        server = await m.start_monitoring_server(
            cfg, health, m.RuntimeState(started_at=1.0), asyncio.Event()
        )
        port = server.sockets[0].getsockname()[1]
        try:
...
class CliChecksTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_check_config_invalid_returns_one_via_run(self):
        with mock.patch(
            "mc_failover_proxy.parse_args",
            return_value=argparse.Namespace(
                config=Path("missing.toml"),
                check_config=True,
                print_effective_config=False,
                test_main=False,
                test_fallback=False,
    ...
    async def test_test_main_uses_main_target(self):
        ...
        with mock.patch(
            "mc_failover_proxy.tcp_health_check",
            new=mock.AsyncMock(return_value=m.HealthCheckResult(True, "ok", 1.2)),
        ) as mocked_tcp:
            self.assertEqual(await m.run_cli_checks(args, cfg), 0)
            mocked_tcp.assert_awaited_once_with(
                cfg.main.host, cfg.main.port, cfg.connection.timeout_seconds
            )
    ...
    async def test_test_fallback_uses_fallback_target(self):
        ...
        with mock.patch(
            ...
            self.assertEqual(await m.run_cli_checks(args, cfg), 0)
            mocked_tcp.assert_awaited_once_with(
                cfg.fallback.host, cfg.fallback.port, cfg.connection.timeout_seconds
            )
    ...
    async def test_test_healthcheck_uses_check_main_server(self):
        ...
        with mock.patch(
            "mc_failover_proxy.check_main_server",
            new=mock.AsyncMock(return_value=m.HealthCheckResult(True, "ok", 1.2)),
        ) as mocked_check:
            self.assertEqual(await m.run_cli_checks(args, cfg), 0)
    ...
    async def test_multiple_flags_fail_if_one_check_fails(self):
        ...
        with mock.patch(
            "mc_failover_proxy.tcp_health_check", new=mock.AsyncMock(side_effect=tcp_side_effect)
        ):
